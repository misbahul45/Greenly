...
import { CartUpdatedPayload } from '../shared/interfaces/cart.interface';
import { CART_ROUTING_KEYS } from '../shared/constants/routing-keys.constant';
...
export class CartUpdatedPublisher {
  ...
  async publish(payload: CartUpdatedPayload): Promise<void> {
    await this.broker.publish(CART_ROUTING_KEYS.CART_UPDATED, {
      ...payload,
      source: 'core-service',
      version: '1.0',
    });
