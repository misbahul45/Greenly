import 'package:flutter/material.dart';

class UIConstants {
  static const double paddingXS = 4.0;
  static const double paddingS = 6.0;
  static const double paddingM = 10.0;
  static const double paddingL = 12.0;
  static const double paddingXL = 16.0;

  static const double spacingXS = 4.0;
  static const double spacingS = 8.0;
  static const double spacingM = 12.0;
  static const double spacingL = 16.0;
  static const double spacingXL = 20.0;
  static const double spacingXXL = 24.0;
  static const double spacingXXXL = 32.0;

  static const double radiusS = 8.0;
  static const double radiusM = 12.0;
  static const double radiusL = 16.0;
  static const double radiusXL = 20.0;
  static const double radiusXXL = 24.0;

  static const double buttonHeight = 50.0;
  static const double logoHeight = 140.0;
  static const double iconSizeS = 16.0;
  static const double iconSizeM = 20.0;
  static const double iconSizeL = 24.0;

  static const double fontSizeXS = 8.0;
  static const double fontSizeS = 10.0;
  static const double fontSizeM = 12.0;
  static const double fontSizeL = 14.0;
  static const double fontSizeXL = 16.0;
  static const double fontSizeXXL = 18.0;

  static const EdgeInsets screenPadding = EdgeInsets.symmetric(
    horizontal: paddingL,
  );
  static const EdgeInsets cardPadding = EdgeInsets.fromLTRB(
    paddingL,
    paddingXXL,
    paddingL,
    paddingXL,
  );
  static const EdgeInsets formFieldSpacing = EdgeInsets.only(bottom: paddingM);

  static const double paddingXXL = 20.0;
  static const double paddingXXXL = 28.0;

  static const double decorCircleLarge = 260.0;
  static const double decorCircleMedium = 220.0;
  static const double decorCircleSmall = 180.0;

  static const double otpIconOuter = 100.0;
  static const double otpIconInner = 72.0;
  static const double otpIconSize = 34.0;

  static const double maxContentWidth = 600.0;

  static const String logoPath = 'assets/images/logo.png';
}
